  ...
  <div class="search">
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="500">
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
        ...
        <FormItem label="跳转地址" prop="urlInfo" style="width: 345px">
          <Input v-model="form.urlInfo" clearable style="width: 100%"/>
        </FormItem>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      getBlindBoxBannerPage(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
        ...
        if (valid) {
          ...
          if (this.modalType === 0) {
            ...
            addBanner(this.form).then((res) => {
              console.log("添加日志："+JSON.stringify(res));
    ...
    enable(v) {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
          disableBanner(v.id, {disable: false}).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getDataList();
    ...
    disable(v) {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
          disableBanner(v.id, {disable: true}).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getDataList();
